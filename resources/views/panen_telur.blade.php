  ...
  <header class="header">
    <nav class="navbar navbar-expand-lg bg-light rounded shadow">
      <div class="container-fluid">
        <a href="#"> <span class="fs-4" style="color: #61431F; padding-left: 10px;"><b>PANEN TELUR</b></span> </a>
...
<script>
  ...
  <script>
    ...
    $(document).on('click', '.editPanentelurBtn', function() {
      ...
      $.ajax({
        url: `https://eggspert.site/api/panentelurku/${panentelurId}`, // Adjust URL if needed
        type: 'GET',
        headers: {
    ...
    $('#edit_panentelur_form').submit(function(e) {
      ...
      $.ajax({
        url: `https://eggspert.site/api/panentelurku/${panentelurId}`, // Send PUT request to update kandang
        type: 'PUT',
        data: formData,
        headers: {
          'Authorization': 'Bearer ' + localStorage.getItem('bearer_token') // Attach Bearer token
...
$(document).on('click', '.deletePanentelurBtn', function(event) {
    ...
    if (confirm('Anda yakin ingin menghapus data panen telur ini?')) {
        ...
        $.ajax({
            url: `https://eggspert.site/api/panentelurku/${panentelurId}`,  // Adjust the URL if necessary
            type: 'DELETE',
            headers: {
