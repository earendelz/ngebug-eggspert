  ...
  <header class="header">
    <nav class="navbar navbar-expand-lg bg-light rounded shadow">
      <div class="container-fluid">
        <a href="#"> <span class="fs-4" style="color: #61431F; padding-left: 10px;"><b>LAPORAN GUDANG</b></span> </a>
...
<script>
  ...
  <script>
    ...
    $(document).on('click', '.editLaporangudangBtn', function() {
      ...
      $.ajax({
        url: `https://eggspert.site/api/laporangudangku/${laporangudangId}`, // Adjust URL if needed
        type: 'GET',
        headers: {
    ...
    $('#edit_laporangudang_form').submit(function(e) {
      ...
      $.ajax({
        url: `https://eggspert.site/api/laporangudangku/${laporangudangId}`, // Send PUT request to update kandang
        type: 'PUT',
        data: formData,
        headers: {
          'Authorization': 'Bearer ' + localStorage.getItem('bearer_token') // Attach Bearer token
...
$(document).on('click', '.deleteLaporangudangBtn', function(event) {
    ...
    if (confirm('Anda yakin ingin menghapus data laporan gudang ini?')) {
        ...
        $.ajax({
            url: `https://eggspert.site/api/laporangudangku/${laporangudangId}`,  // Adjust the URL if necessary
            type: 'DELETE',
            headers: {
