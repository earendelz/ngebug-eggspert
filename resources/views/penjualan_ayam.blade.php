@include ('modals.tambahpenjualanayam')
@include ('modals.editpenjualanayam')
...
<body>
  ...
  <div class="d-flex flex-column flex-shrink-0 p-3 text-white bg-light shadow" style="width: 240px; height: 100vh;">
    ...
    <ul class="nav nav-pills flex-column mb-auto" style="margin-top: 20px;">
        ...
        <li>
          <a href="{{route('penjualan-ayam-dashboard.index')}}" class="nav-link active" aria-current="page">
            <img src="../assets/sidebar/hov_penjualan_ayam.svg" class="nav-img" alt="Penjualan Ayam">
            Penjualan Ayam
          </a>
        </li>
        <li>
  ...
  <header class="header">
    <nav class="navbar navbar-expand-lg bg-light rounded shadow">
      <div class="container-fluid">
        <a href="#"> <span class="fs-4" style="color: #61431F; padding-left: 10px;"><b>PENJUALAN AYAM</b></span> </a>
  ...
  <div class="main-content">
    ...
    <button type="button" id="buttonTambah" class="btn btn-md" data-bs-toggle="modal" data-bs-target="#form_tambah_penjualanayam">
    Tambah Penjualan Ayam
  </button>
</div>
  </div>
...
<script>
  ...
  <script>
    ...
    $.ajax({
        url: "{{ route('penjualanayamku.index') }}", // Your protected API route with the userId
        type: 'GET',
        headers: {
            'Authorization': 'Bearer ' + localStorage.getItem('bearer_token') // Attach the Bearer token
        },
        success: function(response) {
           ...
           // Check if the response is an array (which it should be based on your response)
            if (Array.isArray(response)) {
              var filteredData = response.map((penjualanayamku, index) => {
                    return [
                        index + 1, // Auto-increment ID
                        penjualanayamku.kandang.nama, 
                        penjualanayamku.jumlah_terjual, 
                        penjualanayamku.harga_perekor, 
                        penjualanayamku.harga_total,
                        penjualanayamku.tanggal_penjualan,
                        `<a href="#" data-bs-toggle="modal" data-bs-target="#form_edit_penjualanayam" class="editPenjualanayamBtn" data-id="${penjualanayamku.id}">
                            <img src="../assets/edit_button.svg" alt="Edit">
                        </a>
                        <a href="#" class="deletePenjualanayamBtn" data-id="${penjualanayamku.id}">
  ...
  <script>
    ...
    $(document).on('click', '.editPenjualanayamBtn', function() {
      var penjualanayamId = $(this).data('id'); // Get the ID of the kandang to edit

      // Make AJAX request to fetch kandang details
      $.ajax({
        url: `http://127.0.0.1:8000/api/penjualanayamku/${penjualanayamId}`, // Adjust URL if needed
        type: 'GET',
        headers: {
        ...
        success: function(response) {
          response = response[0];
          $('#idPenjualanayam').val(penjualanayamId);
          // Step 3: Populate modal with the fetched data
         loadKandangData(response.kandang.id);
          $('#ejumlahTerjual').val(response.jumlah_terjual);
          $('#ehargaPerekor').val(response.harga_perekor);
          $('#ehargaTotal').val(response.harga_total);
    ...
    $('#edit_penjualanayam_form').submit(function(e) {
      e.preventDefault(); // Prevent default form submission
      var penjualanayamId = $('#idPenjualanayam').val(); 
      ...
      var formData = {
                      ...
                      jumlah_terjual: $('#ejumlahTerjual').val(),
                      harga_perekor: $('#ehargaPerekor').val(),
      ...
      $.ajax({
        url: `http://127.0.0.1:8000/api/penjualanayamku/${penjualanayamId}`, // Send PUT request to update kandang
        type: 'PUT',
        data: formData,
        headers: {
          'Authorization': 'Bearer ' + localStorage.getItem('bearer_token') // Attach Bearer token
...
<!-- ngehapus -->
<script>
$(document).on('click', '.deletePenjualanayamBtn', function(event) {
    event.preventDefault();  // Prevent the default anchor behavior

    // Get the ID of the kandang to delete
    var penjualanayamId = $(this).data('id');

    // Confirm with the user before deleting
    if (confirm('Anda yakin ingin menghapus data panen telur ini?')) {
        // Send AJAX request to delete the kandang
        $.ajax({
            url: `http://127.0.0.1:8000/api/penjualanayamku/${penjualanayamId}`,  // Adjust the URL if necessary
            type: 'DELETE',
            headers: {
            ...
            success: function(response) {
                ...
                $(`a[data-id="${penjualanayamId}"]`).closest('tr').remove();
                
                alert('Data Penjualan Ayam telah berhasil dihapus!');
            },
            error: function(xhr, status, error) {
